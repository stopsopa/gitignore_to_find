...
async function stripTypes(filePath, isDryRun = false) {
  try {
    ...
    // Rewrite .ts imports/exports to .js for native browser ESM support
    // This handles both single and double quotes, and ensures we only match relative paths ending in .ts
    outputText = outputText.replace(/((?:import|export)\s+.*?\s+from\s+['"])(\.\.?\/.*)\.ts(['"])/g, "$1$2.js$3");
    outputText = outputText.replace(/((?:import|export)\s+['"])(\.\.?\/.*)\.ts(['"])/g, "$1$2.js$3");
